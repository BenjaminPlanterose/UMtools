                            ...
                            deltaposHeightLabel = -0.2, chrHeightLabel = -2)
{
  ...
  phic = Sushi::plotHic(dat, sub_annot$chr, as.integer(rownames(dat)[1]), as.integer(rownames(dat)[nrow(dat)]),
                 max_y, palette = colorspace::diverge_hsv, flip = FALSE)
  Sushi::addlegend(phic[[1]], palette=phic[[2]], title="", side="right", bottominset=0.4, topinset=0,
